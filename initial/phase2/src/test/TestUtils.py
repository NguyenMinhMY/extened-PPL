...
MARSMIPS_JAR = "./external/Mars4_5.jar"
...
class TestCodeGen():
    ...
    def check(soldir,asttree,num):
        ...
        try:
            ...
            subprocess.call("java -jar " + MARSMIPS_JAR + " " + path + "/MT22Class.asm", shell=True, stderr=subprocess.STDOUT, stdout=f)
            # subprocess.run("java -cp ./lib;. MT22Class",shell=True, stdout = f, timeout=10)
            f1 = open(os.path.join(soldir, str(num) + ".txt"),"r")
            rawStr = f1.read()
            f1.close()
            rawStr = rawStr.split("\n")
            f.close()
